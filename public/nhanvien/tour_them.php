...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // ===== 2. Validate KIỂU DỮ LIỆU & LOGIC (Chỉ chạy khi đã nhập đủ) =====
    if (empty($errors)) {
        ...
        if (!is_numeric($old['PhanTramGiam']) || (float)$old['PhanTramGiam'] < 0 || (float)$old['PhanTramGiam'] > 100)
            $errors[] = "Phần trăm giảm phải từ 0 - 100.";

        // Validate Ngày
        if ($old['NgayKhoiHanh'] <= $today) {
            $errors[] = "Ngày khởi hành phải lớn hơn ngày hiện tại.";
        }
        if ($old['NgayKetThuc'] <= $today) {
            $errors[] = "Ngày kết thúc phải lớn hơn ngày hiện tại.";
        }
    ...
    // ===== 3. Validate Ảnh (BẮT BUỘC KHI THÊM MỚI) =====
    $hasFile = isset($_FILES['AnhChinh']) && $_FILES['AnhChinh']['error'] !== UPLOAD_ERR_NO_FILE;
    if (!$hasFile) {
        $errors[] = "Vui lòng chọn 1 ảnh chính cho tour.";
    } else {
    ...
    // ===== 4. Validate Lịch trình (BẮT BUỘC) =====
    ...
    for ($i = 0; $i < $maxRows; $i++) {
        ...
        if ($ng === '' && $td === '' && $nd === '') continue; // Dòng trống bỏ qua

        // Nếu dòng có dữ liệu nhưng thiếu -> Lỗi
        if ($ng === '' || $td === '' || $nd === '') {
            $errors[] = "Lịch trình dòng " . ($i + 1) . ": Vui lòng nhập đủ Ngày thứ, Tiêu đề và Nội dung.";
            continue;
        }
        if (!ctype_digit($ng) || (int)$ng <= 0) {
            $errors[] = "Lịch trình dòng " . ($i + 1) . ": Ngày thứ phải là số nguyên > 0.";
            continue;
        }
    ...
    if (empty($errors)) {
        ...
        try {
            ...
            // 1. Insert Tour
            $sqlTour = "INSERT INTO tour 
                (TenTour, DiaDiem, GiaGoc, GiaGiam, ThoiLuong, NgayKhoiHanh, NgayKetThuc, SoCho, SoChoDaDat, Mien, LoaiTour, PhanTramGiam, TrangThai)
                VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)";

            $stmt = $conn->prepare($sqlTour);
            // Types: s s d d s s s i i s s s s (13 params)
            $stmt->bind_param("ssddsssiissss", $ten, $dd, $g0, $gg, $tl, $ngayKH, $ngayKT, $sc, $scdd, $mien, $loai, $pt, $tt);
            $stmt->execute();
            $maTourNew = (int)$conn->insert_id;
            $stmt->close();
            ...
            $ext = strtolower(pathinfo($_FILES['AnhChinh']['name'], PATHINFO_EXTENSION));
            $safeName = 'tour_' . $maTourNew . '_' . time() . '.' . $ext;
            $destAbs = $uploadDir . $safeName;

            if (!move_uploaded_file($_FILES['AnhChinh']['tmp_name'], $destAbs)) {
            ...
            $stmt = $conn->prepare($sqlLT);
            foreach ($lichtrinh as $lt) {
                $stmt->bind_param("issi", $lt['NgayThu'], $lt['TieuDe'], $lt['NoiDung'], $maTourNew);
                $stmt->execute();
            }
            $stmt->close();

            $conn->commit();
            header("Location: tour.php?msg=added");
            exit;
        } catch (Throwable $e) {
            $conn->rollback();
...
<head>
    <meta charset="utf-8">
    <title>Thêm Tour Mới | VietJourney</title>
...
<body>
        ...
        <main class="main-content">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <div class="text-muted">Tạo tour + ảnh chính + lịch trình</div>
                </div>
            ...
            <div class="card p-4" style="border-radius:16px;">
                <form method="POST" enctype="multipart/form-data" class="row g-3">
                    ...
                    <div class="col-12">
                        <div class="fw-bold text-primary">1) Thông tin tour</div>
                        <hr>
                    </div>

                    <div class="col-md-8">
                        <label class="form-label fw-semibold">Tên Tour <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" name="TenTour" value="<?= h($old['TenTour']) ?>" required>
                    </div>

                    <div class="col-md-4">
                        <label class="form-label fw-semibold">Loại tour <span class="text-danger">*</span></label>
                        <select class="form-select" name="LoaiTour">
                            <option value="Cá nhân" <?= $old['LoaiTour'] === 'Cá nhân' ? 'selected' : ''; ?>>Cá nhân</option>
                            <option value="Doanh nghiệp" <?= $old['LoaiTour'] === 'Doanh nghiệp' ? 'selected' : ''; ?>>Doanh nghiệp</option>
                        </select>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Địa điểm <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" name="DiaDiem" value="<?= h($old['DiaDiem']) ?>" required>
                    </div>

                    <div class="col-md-3">
                        <label class="form-label fw-semibold">Thời lượng <span class="text-danger">*</span></label>
                        <input type="text" class="form-control" name="ThoiLuong" value="<?= h($old['ThoiLuong']) ?>" placeholder="VD: 3N2Đ" required>
                    </div>

                    <div class="col-md-3">
                        <label class="form-label fw-semibold">Số chỗ <span class="text-danger">*</span></label>
                        <input type="number" class="form-control" name="SoCho" value="<?= h($old['SoCho']) ?>" min="1" required>
                    </div>
                    ...
                    <div class="col-12">
                        <div class="fw-bold text-primary mt-2">2) Giá & Ngày</div>
                        <hr>
                    </div>

                    <div class="col-md-4">
                        <label class="form-label fw-semibold">Giá gốc (VNĐ) <span class="text-danger">*</span></label>
                        <input type="number" class="form-control" name="GiaGoc" id="GiaGoc" value="<?= h($old['GiaGoc']) ?>" min="0" required>
                    </div>

                    <div class="col-md-4">
                        <label class="form-label fw-semibold">Phần trăm giảm (%) <span class="text-danger">*</span></label>
                        <input type="number" class="form-control" name="PhanTramGiam" id="PhanTram" value="<?= h($old['PhanTramGiam']) ?>" min="0" max="100" required>
                    </div>

                    <div class="col-md-4">
                        <label class="form-label fw-semibold">Giá giảm (VNĐ) <span class="text-danger">*</span></label>
                        <input type="number" class="form-control" name="GiaGiam" id="GiaGiam" value="<?= h($old['GiaGiam']) ?>" min="0" required>
                    ...
                    <div class="col-md-6">
                        ...
                        <input type="date" class="form-control" name="NgayKhoiHanh" id="NgayKhoiHanh" value="<?= h($old['NgayKhoiHanh']) ?>" min="<?= h($minDate) ?>" required>
                        <div class="form-text">Không được chọn ngày ≤ hôm nay</div>
                    </div>

                    <div class="col-md-6">
                        <label class="form-label fw-semibold">Ngày kết thúc <span class="text-danger">*</span></label>
                        <input type="date" class="form-control" name="NgayKetThuc" id="NgayKetThuc" value="<?= h($old['NgayKetThuc']) ?>" min="<?= h($minDate) ?>" required>
                        <div class="form-text">Không được chọn ngày ≤ hôm nay, và phải ≥ ngày khởi hành</div>
                    </div>

                    <div class="col-12">
                        <div class="fw-bold text-primary mt-2">3) Lịch trình tour</div>
                        <hr>
                    </div>

                    <div class="col-12 d-flex justify-content-between align-items-center">
                        <div class="text-muted">Thêm các dòng lịch trình (Bắt buộc phải nhập)</div>
                            ...
                            <table class="table table-bordered align-middle" id="tblLT">
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td><textarea class="form-control" name="lt_noidung[]" rows="2" placeholder="Mô tả..." required></textarea></td>
                                        <td class="text-center">
                                            <button type="button" class="btn btn-sm btn-outline-danger btnDelLT">
                    ...
                    <div class="col-12 d-flex justify-content-end gap-2 mt-2">
                        <a href="tour.php" class="btn btn-light border px-4">Hủy</a>
    ...
    <script>
        ...
        if (inpGiaGoc && inpPhanTram && inpGiaGiam) {
        ...
        function syncMinEnd() {
            if (ngayKH && ngayKT && ngayKH.value) {
                ngayKT.min = ngayKH.value;
            }
        }
        if (ngayKH && ngayKT) {
        ...
        // === 3. LỊCH TRÌNH TOUR (Cải tiến: Tự tăng ngày) ===
        ...
        function bindDelete() {
            document.querySelectorAll(".btnDelLT").forEach(btn => {
                btn.onclick = () => {
                    // Cho phép xóa thoải mái, nhưng nếu chỉ còn 1 dòng thì giữ lại để user nhập
                    if (tblBody.querySelectorAll("tr").length > 1) {
                        btn.closest("tr").remove();
                    } else {
                        alert('Phải có ít nhất 1 dòng lịch trình!');
                    }
                };
            });
        }
